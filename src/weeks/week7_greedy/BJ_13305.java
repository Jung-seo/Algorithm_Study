...
public class BJ_13305 { // 주유소 // 58점
    public static void main(String[] args) throws IOException {
        ...
        for (int i = 0; i < dis.length - 1; i++) {
            long curOil = Long.parseLong(oil[i]);
            long nextOil = Long.parseLong(oil[i + 1]);
            long distance = Long.parseLong(dis[i]); // 다음 도시까지 가야하는 거리

            if (curOil > nextOil) { // 다음 주유소 가격이 더 쌀때
                price += distance * curOil;
            }
            else { // 지금 가격이 다음 가격보다 쌀때
                distance = 0; // j 인덱스 도시까지 가야하는 거리
                for (int j = i + 1; j < oil.length; j++) { // 더 싼 주유소가 나올때까지 거리를 더함
                    nextOil = Long.parseLong(oil[j]); // 다음에 나올 기름 가격

                    if (nextOil < curOil || j == oil.length - 1) { // 더 싼 주유소가 나오거나 마지막까지
...
// distance[1] 만큼의 기름 충전 -> distance 남은 요소들 만큼 기름 충전